...
regP_1 = smf.ols(formula='np.log10(peak_P_strain) ~ magnitude + np.log10(distance_in_km)', data=peak_amplitude_df).fit()
regS_1 = smf.ols(formula='np.log10(peak_S_strain) ~ magnitude + np.log10(distance_in_km)', data=peak_amplitude_df).fit()
...
plot_compare_prediction_vs_true_values(peak_amplitude_df, y_P_predict_1, y_S_predict_1, (1.0, 8.5), 
regression_results_dir + '/validate_predicted_strain_all_events_no_site_terms.png', type='strain')

# %% Regression 2: Linear regression on the data point including the site term
regP_2 = smf.ols(formula='np.log10(peak_P_strain) ~ magnitude + np.log10(distance_in_km) + C(channel_id) - 1', data=peak_amplitude_df).fit()
regS_2 = smf.ols(formula='np.log10(peak_S_strain) ~ magnitude + np.log10(distance_in_km) + C(channel_id) - 1', data=peak_amplitude_df).fit()
...
# Compare Ground truth values
plot_compare_prediction_vs_true_values(peak_amplitude_df, y_P_predict_2, y_S_predict_2, (1.0, 8.5), 
regression_results_dir + '/validate_predicted_strain_all_events_with_site_terms.png', type='strain')
...
for nearby_channel_number in [10, 20, 50, 100]:
    ...
    regP_3 = smf.ols(formula='np.log10(peak_P_strain) ~ magnitude + np.log10(distance_in_km) + C(combined_channel_id) - 1', data=peak_amplitude_df).fit()
    regS_3 = smf.ols(formula='np.log10(peak_S_strain) ~ magnitude + np.log10(distance_in_km) + C(combined_channel_id) - 1', data=peak_amplitude_df).fit()
    ...
    plot_compare_prediction_vs_true_values(peak_amplitude_df, y_P_predict_3, y_S_predict_3, (1.0, 8.5), 
    regression_results_dir + f'/validate_predicted_strain_all_events_with_combined_site_terms_{nearby_channel_number}chan.png', type='strain')
