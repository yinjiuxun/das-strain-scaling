...
from utility.processing import combined_channels, load_and_add_region
from utility.regression import fit_regression, secondary_site_calibration, store_regression_results
# %% 
# Define Step Functions used here
def combined_regions_for_regression(peak_file_list, apply_calibrated_distance=True):
    """Preprocess the data file: combining different channels etc."""
    peak_data_list = []
    das_index_list = []

    for peak_df in peak_file_list:
        peak_amplitude_df, DAS_index = load_and_add_region(peak_df, region_label='mammothS', 
...
if preprocess_needed:
    combined_regions_for_regression(peak_file_list)
#%% 
...
for nearby_channel_number in combined_channel_number_list:
    ...
    regP, regS = fit_regression(peak_amplitude_df, regression_results_dir, regression_results_filename=results_file_name,
                                weighted='ols', M_threshold=M_threshold, snr_threshold=snr_threshold, min_channel=min_channel)
    ...
    #TODO: add site term output here!
...
for nearby_channel_number in combined_channel_number_list:
    ...
    results_file_name = f"regression_combined_site_terms_{nearby_channel_number}chan"
    regP, regS = fit_regression(peak_amplitude_df, regression_results_dir, regression_results_filename=results_file_name,
                                weighted='wls', M_threshold=M_threshold, snr_threshold=snr_threshold, min_channel=min_channel)
...
#TODO: add combineing fitting parameters here and output a table!
