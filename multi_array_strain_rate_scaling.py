...
if preprocess_needed:
    combined_regions_for_regression(peak_file_list, region_list, combined_channel_number_list, results_output_dir,
                                    snr_threshold, M_threshold, apply_calibrated_distance=True)
...
peak_amplitude_df = pd.read_csv(results_output_dir + f'/peak_amplitude_region_site_-1.csv')
if 'region_site' not in peak_amplitude_df.keys(): # make it work on single array as well
    peak_amplitude_df['region_site'] = 'single_array'
...
# peak_amplitude_df = pd.read_csv(results_output_dir + f'/peak_amplitude_region_site_-1.csv')
# if 'region_site' not in peak_amplitude_df.keys():
#     peak_amplitude_df['region_site'] = 'single_array'
