...
def split_P_S_dataframe(peak_amplitude_df):
    ...
    peak_amplitude_df_P = peak_amplitude_df[['peak_P', 'magnitude', 'calibrated_distance_in_km', 'region_site']]
    peak_amplitude_df_S = peak_amplitude_df[['peak_S', 'magnitude', 'calibrated_distance_in_km', 'region_site']]
...
def fit_regression_magnitude_range(peak_amplitude_df, M_threshold, regression_results_dir, nearby_channel_number):
    ...
    regP = smf.ols(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region_site) - 1', data=peak_amplitude_df_P).fit()
    regS = smf.ols(formula='np.log10(peak_S) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region_site) - 1', data=peak_amplitude_df_S).fit()
...
def fit_regression_with_weight_magnitude_range(peak_amplitude_df, M_threshold, regression_results_dir, nearby_channel_number):
    ...
    regP = smf.wls(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region_site) - 1', 
                data=peak_amplitude_df_P, weights = (10**peak_amplitude_df_P.magnitude)).fit()
    regS = smf.wls(formula='np.log10(peak_S) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region_site) - 1', 
...
def fit_regression_with_attenuation_magnitude_range(peak_amplitude_df, M_threshold, regression_results_dir, nearby_channel_number):
    ...
    regP = smf.ols(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region):distance_in_km + C(region_site) - 1', data=peak_amplitude_df_P).fit()
    regS = smf.ols(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(region):distance_in_km + C(region_site) - 1', data=peak_amplitude_df_S).fit()
