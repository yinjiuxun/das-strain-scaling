...
dx, dy = 5, 5
nx, ny = 24, 41
eq_domain_x = np.linspace(-nx*dx, 0, nx, endpoint=False)
eq_domain_y = np.linspace(-(ny-1)*dy/2, (ny-1)*dy/2, ny, endpoint=True)
...
das_y0 = np.repeat([np.ones(das_x0.shape[1])], n_das, axis=0) * np.linspace(-(ny-1)*dy/2, (ny-1)*dy/2, n_das)[:, np.newaxis]
das_label = 'multiple_arrays'

das_x = das_x0.flatten()
das_y = das_y0.flatten()

# das_y = np.arange(-n_channel/2, n_channel/2)*dx_das
# das_x = np.zeros(das_y.shape)-10
# das_label = 'vertical'

# make the station distribution 
dx_st, dy_st = 10, 10
st_domain_x = np.arange(0, 100+dx_st, dx_st)
st_domain_y = np.arange(-(ny-1)*dy/2, (ny-1)*dy/2+dy_st, dy_st)
#%%
...
cmap=cmap, vmin=-20, vmax=20)
ax.contour(eq_x_grid, eq_y_grid, diff_response_time, levels=[0])

ax.plot(das_x0[0, :], das_y0.T, '-r', linewidth=3)
...
ax.vlines(x=st_domain_x.min()-100, ymax=eq_domain_y.max(), ymin=eq_domain_y.min(), color='k', linestyle='-.', linewidth=2)
...
ax.text(eq_domain_x.min()+10, eq_domain_y.max()-40, f'source depth:\n{eq_depth}km')
...
plt.savefig(fig_output_dir + f'/{das_label}_DAS_depth_{eq_depth}kmx.png', bbox_inches='tight')
# plt.close('all')
# %%
# # map of source, station, and DAS
# plt.scatter(eq_x_grid.flatten(), eq_y_grid.flatten(),c=DAS_response_time, s=10, cmap='jet')
