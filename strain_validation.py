...
for ii in range(len(peak_file_name_list)):
    ...
    peak_comparison_df = get_comparison_df(data=[peak_amplitude_df_temp.event_id, peak_amplitude_df_temp.peak_P, peak_amplitude_df_temp.peak_S, peak_P_predicted, peak_S_predicted], 
                                            columns=['event_id', 'peak_P', 'peak_S', 'peak_P_predict', 'peak_S_predict'])
    ...
    if result_label == 'iter': 
        gP = plot_prediction_vs_measure_seaborn(peak_comparison_df, phase='P', bins=40, vmax=vmax, xlim=xy_lim, ylim=xy_lim, height=height, space=space)
        gP.savefig(fig_dir + f'/P_validate_strain_rate_{result_label}_seaborn.png')
        gS = plot_prediction_vs_measure_seaborn(peak_comparison_df, phase='S', bins=40, vmax=vmax, xlim=xy_lim, ylim=xy_lim, height=height, space=space)
        ...
        final_peak_fit = peak_comparison_df[peak_comparison_df.event_id.isin(event_id_fit)]
        final_peak_predict = peak_comparison_df[peak_comparison_df.event_id.isin(event_id_predict)]
