...
def split_P_S_dataframe(peak_amplitude_df):
    ...
    peak_amplitude_df_P = peak_amplitude_df[['peak_P', 'magnitude', 'calibrated_distance_in_km', 'combined_channel_id']]
    peak_amplitude_df_S = peak_amplitude_df[['peak_S', 'magnitude', 'calibrated_distance_in_km', 'combined_channel_id']]
...
def fit_regression_magnitude_range(peak_amplitude_df, M_threshold, regression_results_dir, nearby_channel_number, given_coefficients=None):
    ...
    if given_coefficients is None:
        ...
        if peak_amplitude_df_P.shape[0] != 0:
            regP = smf.ols(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(combined_channel_id) - 1', data=peak_amplitude_df_P).fit()
            print(regP.params[-2:])
        ...
        if peak_amplitude_df_S.shape[0] != 0:
            regS = smf.ols(formula='np.log10(peak_S) ~ magnitude + np.log10(calibrated_distance_in_km) + C(combined_channel_id) - 1', data=peak_amplitude_df_S).fit()      
            print(regS.params[-2:])
        ...
        if (~np.isnan(given_coefficients[0])) & (~np.isnan(given_coefficients[1])):
            ...
            peak_amplitude_df_P.peak_P = peak_amplitude_df_P.peak_P / 10**(peak_amplitude_df_P.magnitude * mag_coef_P) / peak_amplitude_df_P.calibrated_distance_in_km**dist_coef_P
        ...
        if (~np.isnan(given_coefficients[2])) & (~np.isnan(given_coefficients[3])):
            ...
            peak_amplitude_df_S.peak_S = peak_amplitude_df_S.peak_S / 10**(peak_amplitude_df_S.magnitude * mag_coef_S) / peak_amplitude_df_S.calibrated_distance_in_km**dist_coef_S
...
def fit_regression_with_weight_magnitude_range(peak_amplitude_df, M_threshold, regression_results_dir, nearby_channel_number, given_coefficients=None):
    ...
    if given_coefficients is None:
        print('Fit for all coefficients........\n')
        if peak_amplitude_df_P.shape[0] != 0:
            regP = smf.wls(formula='np.log10(peak_P) ~ magnitude + np.log10(calibrated_distance_in_km) + C(combined_channel_id) - 1', 
        ...
        if peak_amplitude_df_S.shape[0] != 0:
            regS = smf.wls(formula='np.log10(peak_S) ~ magnitude + np.log10(calibrated_distance_in_km) + C(combined_channel_id) - 1', 
...
if region_label == 'sanriku':
    given_coefficients_list = [[np.nan, np.nan, 0.628, -1.314],
                               [np.nan, np.nan, 0.632, -1.324],
                               [np.nan, np.nan, 0.639, -1.342],
                               [np.nan, np.nan, 0.644, -1.357]]

    nearby_channel_number_list = [100, 50, 20, 10]

    regression_results_dir = results_output_dir + '/regression_results_smf_calibrated'
...
if region_label == 'sanriku':
    given_coefficients_list = [[np.nan, np.nan, 0.590, -1.296],
                               [np.nan, np.nan, 0.594, -1.312],
                               [np.nan, np.nan, 0.606, -1.349],
                               [np.nan, np.nan, 0.618, -1.384]]

    nearby_channel_number_list = [100, 50, 20, 10]

    regression_results_dir = results_output_dir + '/regression_results_smf_weighted_calibrated'
    if not os.path.exists(regression_results_dir):
        os.mkdir(regression_results_dir)
...
if region_label == 'sanriku':
    ...
    regression_results_dir = results_output_dir + '/regression_results_smf_all_coefficients_calibrated'
    if not os.path.exists(regression_results_dir):
        os.mkdir(regression_results_dir)
...
if region_label == 'sanriku':
    ...
    regression_results_dir = results_output_dir + '/regression_results_smf_weighted_all_coefficients_calibrated'
    if not os.path.exists(regression_results_dir):
        os.mkdir(regression_results_dir)
...
if region_label == 'sanriku':
    ...
    regression_results_dir = results_output_dir + '/regression_results_smf_all_coefficients_drop_4130_calibrated'
    if not os.path.exists(regression_results_dir):
        os.mkdir(regression_results_dir)
...
if region_label == 'sanriku':
    ...
    regression_results_dir = results_output_dir + '/regression_results_smf_weighted_all_coefficients_drop_4130_calibrated'
    if not os.path.exists(regression_results_dir):
        os.mkdir(regression_results_dir)
...
given_coefficients_list = [[0.590, -1.296],
                            [0.594, -1.312],
                            [0.606, -1.349],
                            [0.618, -1.384]]
