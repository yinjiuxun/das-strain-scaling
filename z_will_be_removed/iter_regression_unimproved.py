...
M_threshold_list = [[0, 10], [0, 10], [0, 10]]
...
M_threshold_list = [[0, 10], [0, 10], [0, 10], [0, 10], [0, 10]]
snr_threshold_list = [5, 5, 5, 5, 5]
min_channel = 100
...
for i_region in [4]:
    ...
    regression_results_dir = results_output_dir + f'/iter_regression_results_smf{weight_text}_{min_channel}_channel_at_least_snr5_M0_no_clipped_events_no_close_small'
    # So-called unimproved: no magnitude threshold for LAX and Sanriku, keep the clipped events
    mkdir(regression_results_dir)

    peak_amplitude_df = pd.read_csv(results_output_dir + '/peak_amplitude_events/calibrated_peak_amplitude.csv')
    peak_amplitude_df['distance_in_km'] = peak_amplitude_df['calibrated_distance_in_km']
    
    # Drop the clipped events
    ...
    if 'Sanriku' in results_output_dir: # some special processing for Sanriku data
        ...
        # some incorrectly associated events
        peak_amplitude_df = peak_amplitude_df.drop(index=peak_amplitude_df[(peak_amplitude_df.distance_in_km>=100)&(peak_amplitude_df.magnitude<=2)].index)
...
def log_median(x):
    return 10**np.nanmedian(np.log10(x))
...
def bin_average_mag(temp_df_now, scale=False):
    d_mag = 0.25
    mag_bin = np.arange(0, 10, d_mag)
    mag_center = mag_bin[:-1] + d_mag/2

    bin_average_P = np.zeros(mag_center.shape)*np.nan
    bin_average_S = bin_average_P.copy()

    for ii in range(len(mag_center)):
        temp = temp_df_now[(temp_df_now.magnitude >= mag_bin[ii]) & (temp_df_now.magnitude < mag_bin[ii+1])]

        bin_average_P[ii] = log_median(temp.peak_P)
        bin_average_S[ii] = log_median(temp.peak_S)

        if scale:
            bin_average_P[ii] = bin_average_P[ii]/log_median(temp.distance_in_km)
            bin_average_S[ii] = bin_average_S[ii]/log_median(temp.distance_in_km)

    return mag_center, bin_average_P, bin_average_S
...
for ii_region in range(5):
    ...
    temp_df = peak_amplitude_df.groupby(['event_id', 'region']).agg({'peak_P': log_median, 'peak_S': log_median, 'magnitude': 'mean', 'distance_in_km': log_median}).reset_index()
    temp_df[temp_df.peak_P == 0].loc[:, 'peak_P'] = np.nan

    region = temp_df['region'][0]


    mag_center, bin_average_P, bin_average_S = bin_average_mag(temp_df)
    ax[0].semilogy(mag_center, bin_average_P, 'x', alpha=1, color=colors[ii_region], label=region)
    ax[1].semilogy(mag_center, bin_average_S, 'x', alpha=1, color=colors[ii_region], label=region)
    
    mag_center, bin_average_P, bin_average_S = bin_average_mag(temp_df, scale=True)
    ax[2].semilogy(mag_center, bin_average_P, 'x', alpha=1, color=colors[ii_region], label=region)
    ax[3].semilogy(mag_center, bin_average_S, 'x', alpha=1, color=colors[ii_region], label=region)
    ...
    ax[3].set_xlabel('magnitude')

    ax[0].set_ylabel('median P peak amplitude')
    ax[1].set_ylabel('median S peak amplitude')
    ax[2].set_ylabel('median P peak amplitude scaled with distance')
    ax[3].set_ylabel('median S peak amplitude scaled with distance')
    plt.legend()
